...
# NOT YET UPDATED TO WORK WITH NEW BLOCK SPARSE MATRIX
...
#     m = (m + m.conj().T)
#     return DM.DensityMatrix(DM.SPARSE_TYPE(m, dtype=xp.complex64), energy_basis(nqbits))
#
#
...
def random_unitary_in_subspace(nqbits: int, energy_subspace: int):
    ...
    return DM.DensityMatrix(BSM(blocks), energy_basis(nqbits))
...
def random_energy_preserving_unitary(nqbits: int) -> DM.DensityMatrix:
    ...
    """
    blocks = [np.array([[1. + 0j]])] + [unitary_group.rvs(comb(nqbits, i)).astype(np.complex64) for i in range(1, nqbits)] + [np.array([[1. + 0j]])]
