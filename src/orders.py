...
def n_random_c6_orders(num_qbits: int, chunk_sizes: list[int], n: int) -> list[np.ndarray]:
    ...
    return [np.split(np.roll(np.arange(num_qbits), np.random.randint(num_qbits)), split_indices) for _ in range(n)]
...
def n_random_c5_orders(num_qbits: int, n: int) -> list[np.ndarray]:
    ...
    # randomly select between the two possible orders
    return [possible_order_1 if rng.random() < 0.5 else possible_order_2 for _ in range(n)]
...
def n_random_c7_orders(num_qbits: int, n: int) -> list[np.ndarray]:
    ...
    return [rng.choice(groups) for _ in range(n)]
