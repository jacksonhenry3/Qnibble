...
class TestComprehensive:
    ...
    def test_pipeline(self):
        ...
        # rearange the hamiltonian data to be in the energy basis order
        order = [n.num for n in energy_basis(4)]
        hamiltonian_data = hamiltonian_data[order]
        hamiltonian_data = hamiltonian_data[:, order]


        blocks = []
        block_sizes = [1, 4, 6, 4, 1]
        block_index = 0
        for i in range(5):
            blocks.append(hamiltonian_data[block_index:block_index + block_sizes[i], block_index:block_index + block_sizes[i]])
            block_index += block_sizes[i]

        H = DM.DensityMatrix(Block_Sparse_Matrix.BlockSparseMatrix(blocks), energy_basis(block_size))
        ...
        UnnatiResults = xp.array([[0.102022, 0.200982, 0.298861, 0.398135, 0.152104, 0.251058, 0.348878, 0.447961], [0.105157, 0.204515, 0.296364, 0.395547, 0.153089, 0.252141, 0.347254, 0.445933],
        ...
        plt.plot(pops, '.')
        plt.plot(UnnatiResults, 'o', alpha=.2)
        plt.show()
